...
(async()=>{
  ...
  for(const t of todo.slice(0,cap)){
    ...
    if(r.ok&&r.data?.InvoiceNo){ await ref.set({status:'issued',invoice_no:r.data.InvoiceNo,invoice_date:String(r.data.InvoiceDate||'').slice(0,10),issued_at:Date.now()},{merge:true}); ok++; console.log(`  ✓ ${tw(t.at)} NT$${t.amount_twd} → ${r.data.InvoiceNo}(綠界會自動寄信給客人)`); }
