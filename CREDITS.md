# 第三方素材授權標註

## KanjiVG — 假名/漢字筆順資料

本站「五十音筆順」功能使用的筆順 SVG 來自 **KanjiVG**。

- 專案網站：http://kanjivg.tagaini.net
- 版權：Copyright © 2009-2011 Ulrich Apel
- 授權：Creative Commons Attribution-Share Alike 3.0（CC BY-SA 3.0）
  https://creativecommons.org/licenses/by-sa/3.0/

依 CC BY-SA 3.0 之要求：
1. 已保留每個 SVG 原始檔內的版權與授權標頭（見 `assets/kanjivg/kana/*.svg`）。
2. 於使用處（App 內五十音筆順畫面）標示「筆順資料：KanjiVG (CC BY-SA 3.0)」並連結專案網站。
3. 若對筆順資料本身進行修改衍生並散布，將以相同 CC BY-SA 3.0 授權釋出。

存放位置：`assets/kanjivg/kana/`（142 個單字元假名，平假名＋片假名，清音／濁音／半濁音）。

## 文章封面圖 — Openverse（CC0 / 公共領域）

`images/articles/*.jpg` 為文章閱讀清單與 hero 的封面圖，透過 [Openverse](https://openverse.org) 搜尋、僅取 **CC0（公眾領域貢獻）與 Public Domain Mark（PDM）** 授權之圖片。此類授權**可自由商用、無需標註**；此處仍記錄來源以示尊重。已縮為 400×400 JPG。
