...
- 這是開場(還沒有對方的話)時,COACH 留空,由你先自然開口帶起情境。
- 對話自然走到尾聲時,可以帶到道別收尾。
【輸出格式】嚴格逐行輸出,一行一個欄位,不要 JSON、不要多餘文字或旁白:
JP: <你這個角色要講的日文(只有台詞本身)>
KANA: <JP 的整句假名讀音>
ZH: <JP 的繁體中文翻譯>
COACH: <對對方「上一句」的簡短繁中提點或鼓勵;開場或沒必要時留空>`;
}

const REVIEW_SYSTEM = `你是 StayJP 的日語口說教練。下面是一段學生和 AI 的情境對話(JP 標學生說的、AI 標對方角色)。
請用繁體中文,針對「學生」的表現給溫暖但具體的整體點評。嚴格逐行輸出:
SCORE: 0~100 整數(這次對話的口說綜合表現)
GOOD: 一句,講得好的地方(具體)
IMPROVE: 一句,最該加強的地方(助詞/動詞變化/用詞/自然度,具體、學得到)
TIP: 一句,下次馬上能用的小建議
ENCOURAGE: 一句像真人教練的鼓勵`;
